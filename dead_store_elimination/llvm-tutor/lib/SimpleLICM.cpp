...
struct SimpleLICM : public PassInfoMixin<SimpleLICM> {
  PreservedAnalyses run(Loop &L, LoopAnalysisManager &AM,
    ...
    auto isInvariantGivenSet = [&](Instruction &I) -> bool {
      // Skip PHIs and memory operations
      if (isa<PHINode>(I)) return false;
      if (I.mayReadOrWriteMemory()) return false;

      // Optionally skip non-speculative operations
      if (!isSafeToSpeculativelyExecute(&I)) return false;

      // Check that all operands are invariant or come from outside the loop
    ...
    while (Changed) {
      ...
      for (BasicBlock *BB : L.blocks()) {
        for (Instruction &I : *BB) {
          if (!InvariantSet.count(&I) && isInvariantGivenSet(I)) {
